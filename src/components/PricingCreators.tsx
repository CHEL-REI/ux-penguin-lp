...
export default function PricingCreators() {
  const { t } = useLocale();
  const plans = t.pricingCreators.plans;
  return (
              ...
              <Reveal key={p.name} variant="up" delay={i * 120}>
                ...
                >
                  <div className="flex items-center justify-between mb-5">
                    <div
                      className={`text-xs tracking-[0.2em] uppercase ${highlight ? 'text-white/60' : 'text-black/50'}`}
                    >
                      {p.name}
                    </div>
                    {highlight && 'badge' in p && (
                      <span className="text-[10px] font-bold uppercase tracking-[0.14em] px-2.5 py-1 rounded-full bg-[#4a4eda] text-white">
                        {p.badge}
                      </span>
                    )}
                  </div>
                  <div className="flex items-baseline gap-2">
                    <div
                      className={`hero-title text-5xl md:text-6xl font-medium ${highlight ? 'text-white' : 'text-black'}`}
                    >
                      {p.price}
                    </div>
